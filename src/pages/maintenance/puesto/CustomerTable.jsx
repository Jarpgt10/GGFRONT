...
const CustomerTable = ({ columns, headers, reload }) => {
    ...
    return (
        <div className="p-4 bg-[#f6f7f8] rounded-md shadow-[#00000026] shadow-sm">
            <div className="flex flex-col sm:flex-row sm:justify-between md:justify-start items-center mb-4 gap-4 sm:gap-2">
                <input
                    type="text"
                    placeholder="Buscar puesto..."
                    className="px-4 py-2 sm:w-[80%] md:w-auto  border rounded-md shadow-sm focus:outline-none"
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                />
                <div className="flex gap-2">
                    <Icon.reload
                        onClick={() => reload.puestos()}
                        size={25}
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full bg-white shadow-md rounded-lg">
                    ...
                    <tbody>
                        {columns
                            .filter((opt) =>
                                opt.puesto.toLowerCase().includes(searchTerm.toLowerCase())
                            )
                            .map((opt, index) => (
                                <tr key={index} className="border-t">
                                    <td className="py-3 px-4">{opt.id_puesto}</td>
            ...
            <div className="mt-4 flex justify-between items-center gap-5">
                <strong className="flex gap-2">
                    Puestos: <p className="px-2 rounded-md bg-slate-200">{cantidad}</p>
                </strong>
            </div>
            ...
            <Modal
                ...
                title={<div className="flex justify-center">{data ? 'Editar' : 'Crear'} Puesto</div>}
            >
                <ModalPuesto data={data} onClose={handleOnClose} />
            </Modal>
        </div>
    );
};
