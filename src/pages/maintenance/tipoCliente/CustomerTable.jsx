    ...
    return (
        <div className="p-4 bg-[#f6f7f8] rounded-md shadow-[#00000026] shadow-sm">
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full bg-white shadow-md rounded-lg">
                    ...
                    <tbody>
                        {columns
                            .filter((opt) =>
                                opt.tipo_cliente.toLowerCase().includes(searchTerm.toLowerCase())
                            )
                            .map((opt, index) => (
                                <tr key={index} className="border-t">
                                    <td className="py-3 px-4">{opt.id_tipo_cliente}</td>
                                    ...
                                    <td className="py-3 px-4" style={{ backgroundColor: opt.color }}></td>
                                    <td className="py-3 px-4">
                                        <Icon.edit
            ...
            <Modal
                ...
                title={<div className="flex justify-center">{data ? "Editar" : "Crear"} tipo cliente</div>}
            >
                <ModalTipoCliente data={data} onClose={() => handleOnClose()} />
            </Modal>
        </div>
    );
};
